...
export class RouletteComponent implements OnDestroy {
  ...
  ngOnDestroy(): void { this.walletSub?.unsubscribe(); }

  isRed(n: number | null | undefined): boolean { if (n == null) return false; if (n === 0) return false; return RouletteComponent.RED_SET.has(Number(n)); }

  choisirNumero(n: number) { this.betType = 'straight'; this.betValue = '' + n; this.selectedNumber = n; }

  private normalize(angle: number): number { let a = angle % 360; if (a < 0) a += 360; return a; }

  private targetModuloForIndex(idx: number): number { const offset = 0; const angleToTarget = idx * this.sectorAngle; return this.normalize(offset - angleToTarget); }
        ...
        setTimeout(() => {
          ...
          if (this.autoSpinActive) {
            if (this.autoSpinCount !== null) {
              if (this.autoSpinCount > 1) { this.autoSpinCount--; this.jouer(true); } else { this.stopAutoSpin(); }
