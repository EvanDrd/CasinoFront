...
export class SlotMachineComponent implements OnDestroy, AfterViewInit {
  ...
  stopAutoSpin() { this.autoSpinActive = false; this.remainingAutoSpins = null; if (this.autoSpinTimeoutId != null) { clearTimeout(this.autoSpinTimeoutId); this.autoSpinTimeoutId = null; } }
  ...
  private onSpinComplete() {
    ...
    if (this.autoSpinActive) {
      ...
      this.autoSpinTimeoutId = window.setTimeout(() => { this.autoSpinTimeoutId = null; if (!this.enCours && this.autoSpinActive) { this.jouer(); } }, this.autoSpinDelay);
    }
  }
    ...
    setTimeout(() => {
      ...
      if (lastStripEl) {
        const finalOnEnd = () => {
          ...
          this.clearAllTimers();
          setTimeout(() => { this.enCours = false; this.onSpinComplete(); }, 120);
        };
